...
your_env_access_key_var = os.environ.get("AWS_ACCESS_KEY_ID")
your_env_secret_key_var = os.environ.get("AWS_SECRET_ACCESS_KEY")
your_stream_name = "reda-test-video-stream"
...
def get_endpoint_boto():
    client = boto3.client(
        ...
        aws_access_key_id=your_env_access_key_var,
        aws_secret_access_key=your_env_secret_key_var,
    )
    response = client.get_data_endpoint(
        StreamName=your_stream_name, APIName="PUT_MEDIA"
    )
    pp.pprint(response)
...
class gen_request_parameters:
    def __init__(self):
        ...
        result = cv2.VideoWriter('filename.mkv',
        ...
        # localfile = "../a_futuristic_dream.mkv"  # MKV FORMAT OBLIGATOIRE POUR ENVOYER SUR KINESIS
        with open("filename.mkv", "rb") as image:
...
while True:
    ...
    access_key = your_env_access_key_var  # '*************************'
    secret_key = your_env_secret_key_var  # '*************************'
    while True:  # scope
