...
data_dir = 'C:/Users/Bonjour/Etudes&Travail/Projets/PyCharm/DeepLearningSafetyDrive/Data'
...
train_file_num = len(glob(osp.join(data_dir, 'train/*/*.jpg')))  # Nombre de données d’entraînement
...
px.histogram(df, x="classname", color="classname", title="Number of images by categories ").show()

drivers_id = pd.DataFrame((df['subject'].value_counts()).reset_index())
drivers_id.columns = ['driver_id', 'Counts']
px.histogram(drivers_id, x="driver_id", y="Counts", color="driver_id", title="Number of images by subjects ").show()

# Histogramme du nombre d’images par pilote
# Implémentez-le vous-même
px.histogram(df, x='subject', color='subject', title='Number of images by subjects').show()
...
# Implémentez-le vous-même
df['file_path'] = df.apply(lambda x: osp.join(data_dir, 'train', x.classname, x.img), axis=1)
    ...
    def __init__(self, input_size, color_mean, color_std):
        self.data_transform = {
            ...
            'train': A.Compose([
                ...
                A.Normalize(color_mean, color_std),  # Normalisation des informations de couleur
...
def run_one_fold(df_train, df_val, fold, device):
    ...
    scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=args.gamma)  # Planificateur

    save_path = f"{args.model_name}_fold_{fold}.pth"
    best_val_loss, best_preds = train_model(model, dataloaders_dict, criterion, scheduler, optimizer, device,
...
if args.debug:
    results = pd.read_csv('C:/Users/Bonjour/Etudes&Travail/Projets/PyCharm/DeepLearningSafetyDrive/Data/result_v3.csv')
    ...
    # Prétraitement
    df_test['file_path'] = df_test.apply(lambda row: osp.join(data_dir, f'my_test/{row.img}'), axis=1)
    df_test['class_num'] = 0
